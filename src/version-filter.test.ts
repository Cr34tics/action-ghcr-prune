...
describe('versionFilter', () => {
  const Version = (created_at: string, tags: string[]): ContainerVersion => ({
    id: 1,
    name: 'sha256:abc123',
    created_at,
    metadata: {
  ...
  it('should prune all versions matching any criteria', () => {
    const versions = [Version(YEARS_AGO, []), Version(YEARS_AGO, ['latest'])]
